...
SERVER_KEY="./sectest.key"
SERVER_CSR="./sectest.csr"
SERVER_CRT="./sectest.crt"
CA_KEY="./ca.key"
CA_CRT="./cacert.pem"
...
function generate_server_certificate {
    ...
    $OPENSSL_CMD x509 -req -in $SERVER_CSR -CA $CA_CRT -CAkey $CA_KEY -out $SERVER_CRT -CAcreateserial -days 365 -sha512 2>/dev/null
...
cat sectest.key sectest.crt cacert.pem > ../../docker/haproxy/sectest.pem
